...
-- CREATE TABLES
-- the allied specializations: mental health counsellor, registered dietitian, etc
CREATE TABLE tbl_Allied_Type(
	typ_id INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	typ_name VARCHAR(35) NOT NULL
);

-- medical record app or EMR types: ps suite, oscar, accuro, etc
CREATE TABLE tbl_Medical_Record(
	emr_id INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	emr_name VARCHAR(35) NOT NULL
);

-- the status of a remote acces request done by an allied: Yes(the allied is able to remote into that practice's emr), in progress, denied, etc
CREATE TABLE tbl_Remote_Status(
	rem_id INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	rem_name VARCHAR(35) NOT NULL
);

-- the status of the last step #5 in the process of a request: ready, completed, etc
CREATE TABLE tbl_stepfive(
	fiv_id INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	fiv_name VARCHAR(35) NOT NULL
);

-- the employment status: active(with a contract) or inactive(ended contract with the organization)
CREATE TABLE tbl_Employment_Status(
	emp_id INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	emp_name VARCHAR(35) NOT NULL
);

-- the It company information of each practice or clinic (each practice outsources or has a staff for technical services) 
...
-- information about the practice or clinic
...
-- the remote connection details of the practices
...
--  information about the allied professional
...
-- store the affiliation or contract between an allied professional and a practice 
...
-- store information about the progress of a request made by an allied professional to remote into a practice's emr or medical record app
...
SELECT CONCAT(al.all_first_name,' ',al.all_last_name) AS 'Allied Fullname', prc.prc_name AS 'Practice',emr.emr_name AS 'EMR',rem.rem_name AS 'Remote Status',sta.sta_emr_login AS 'Has a EMR Login?',sta.sta_comments AS 'Comments',sta.sta_last_update AS 'Last Update' 
...
SELECT CONCAT(al.all_first_name,' ',al.all_last_name) AS 'Allied Fullname', prc.prc_name AS 'Practice',emr.emr_name AS 'EMR',sta.sta_step1 AS 'Step1',sta.sta_step2 AS 'Step2',sta.sta_step3 AS 'Step3',sta.sta_step4 AS 'Step4',fiv.fiv_name AS 'Step5'
FROM tbl_Practice AS prc
JOIN tbl_Medical_Record AS emr
ON emr.emr_id = prc.prc_emr_id 
JOIN tbl_Practice_Allied_Affiliation AS aff
...
ON fiv.fiv_id = sta.sta_fiv_id
WHERE al.all_emp_id = 1
ORDER by sta.sta_last_update DESC,emr.emr_name;
...
FROM tbl_Remote_Status AS rem
JOIN tbl_Allied_Remote_Status AS sta
ON rem.rem_id = sta.sta_rem_id
JOIN tbl_Practice_Allied_Affiliation AS aff
ON aff.aff_id = sta.sta_aff_id
JOIN tbl_Practice AS prc
ON prc.prc_id = aff.aff_prc_id
JOIN tbl_Medical_Record AS emr
ON emr.emr_id = prc.prc_emr_id 
JOIN tbl_Employment_Status AS emp
ON emp.emp_id = prc.prc_emp_id
WHERE prc.prc_emp_id = 1 AND rem.rem_name = 'Yes'
GROUP BY emr.emr_name;

-- Show how many remote access requests are by status for each allied type or specialization
SELECT typ.typ_name AS 'Allied Type', rem.rem_name AS 'Remote Status', COUNT(sta.sta_rem_id) AS 'Total'
	FROM tbl_Remote_Status AS rem
	JOIN tbl_Allied_Remote_Status AS sta
    ON rem.rem_id = sta.sta_rem_id
    JOIN tbl_Practice_Allied_Affiliation AS aff 
    ...
    JOIN tbl_Allied_Type AS typ 
    ON typ.typ_id = al.all_type_id
    GROUP BY rem.rem_name, typ.typ_name
    ORDER BY typ.typ_name;
